    ...
    implements IInventory, FCITileEntityDataPacketHandler
{
				...
				{				
					...
					else 
					{
						...
						if ( targetTileEntity != null && targetTileEntity instanceof IInventory ) 
						{
							...
							//SC ADDED:
							else if (iTargetBlockID == SCDefs.storageJar.blockID)
							{
								if (targetTileEntity instanceof SCTileEntityStorageJar)
								{									
									if ( ((SCTileEntityStorageJar)targetTileEntity).isStackValidForSlot(0, ejectStack) == false )
									{
										//canProcessStack = false;
										return;
									}	
									
									System.out.println("canProcessStack: " + canProcessStack);
								}
							}
							else if (iTargetBlockID == SCDefs.seedJar.blockID)
							{
								if (targetTileEntity instanceof SCTileEntitySeedJar)
								{									
									if ( ((SCTileEntitySeedJar)targetTileEntity).isStackValidForSlot(0,ejectStack) == false )
									{
										//canProcessStack = false;
										return;
									}	
							...
							if ( canProcessStack )
							{
									...
									iTargetBlockID != FCBetterThanWolves.fcBlockChest.blockID )
								{
									...
									//SC ADDED:
									if (iTargetBlockID == SCDefs.storageJar.blockID)
									{
										if (targetTileEntity instanceof SCTileEntityStorageJar)
										{									
											bFullStackDeposited = FCUtilsInventory.AddItemStackToInventoryInSlotRange2( (IInventory)targetTileEntity, ejectStack, iMinSlotToAddTo, iMaxSlotToAddTo );
										}
									}
									else if (iTargetBlockID == SCDefs.seedJar.blockID)
									{
						...
						System.out.println("Output Blocked: " + m_bOutputBlocked);
					}
				}				
			}
    ...
    private void EjectStack( ItemStack stack )
    {
        ...
        EntityItem entityitem = 
        	(EntityItem) EntityList.createEntityOfType(EntityItem.class, worldObj, 
