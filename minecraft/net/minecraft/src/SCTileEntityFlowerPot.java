...
public class SCTileEntityFlowerPot extends TileEntity implements FCITileEntityDataPacketHandler {
	...
	public void placeItemInPot(int itemID, int metadata) {
		...
		else if (itemID == SCDefs.bambooItem.itemID)
			currentBlockID = SCDefs.bambooRoot.blockID;
		else
	...
	public void retrieveItemFromPot(EntityPlayer player) {
		...
		if (!player.capabilities.isCreativeMode)
			if (currentBlockID == SCDefs.bambooRoot.blockID)
			{
				FCUtilsItem.GivePlayerStackOrEjectFavorEmptyHand(player, new ItemStack(SCDefs.bambooItem, 1, 0), this.xCoord, this.yCoord, this.zCoord);
			}
			else FCUtilsItem.GivePlayerStackOrEjectFavorEmptyHand(player, new ItemStack(Item.itemsList[currentBlockID], 1, currentBlockMetadata), this.xCoord, this.yCoord, this.zCoord);
