...
public class SCBlockPumpkinYellowFresh extends SCBlockPumpkinBase {

	protected int stemBlock;
	protected int vineBlock;
	protected int flowerBlock;
	
	protected SCBlockPumpkinYellowFresh(int iBlockID, int stemBlock, int vineBlock, int flowerBlock) {
	...
	public boolean RenderBlock(RenderBlocks renderer, int i, int j, int k) {
		...
		if (growthLevel == 0) {
			renderer.setRenderBounds(GetPumpkinBounds(2/16D, 4/16D)); // stage 0 = 6x6x6
			renderer.renderStandardBlock( this, i, j, k );
		}else if (growthLevel == 1){
			renderer.setRenderBounds(GetPumpkinBounds(3/16D, 6/16D)); // stage 1 = 8x8x8
			renderer.renderStandardBlock( this, i, j, k );
		}else if (growthLevel == 2){
			renderer.setRenderBounds(GetPumpkinBounds(4/16D, 8/16D)); // stage 2 = 12x12x12
			renderer.renderStandardBlock( this, i, j, k );
...
//    private Icon[] connectorIcon;
    ...
    public void registerIcons( IconRegister register )
    {
        ...
        {
            m_iconArray[iTempIndex] = register.registerIcon( "SCBlockPumpkinYellowSide_" + iTempIndex );
        }
        ...
        for ( int iTempIndex = 0; iTempIndex < m_iconArray.length; iTempIndex++ )
        {
            m_iconTopArray[iTempIndex] = register.registerIcon( "SCBlockPumpkinYellowTop_" + iTempIndex );
        }
