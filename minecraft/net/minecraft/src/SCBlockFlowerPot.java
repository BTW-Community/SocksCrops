...
public class SCBlockFlowerPot extends BlockContainer {
	public SCBlockFlowerPot(int id) {
    ...
    {
    	SCTileEntityFlowerPot potTile = (SCTileEntityFlowerPot) world.getBlockTileEntity(x, y, z);
    ...
    {
        return new SCTileEntityFlowerPot();
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
    	...
    	else if (potTile.isValidItemForPot(heldID, heldDamage) || 
    			heldID == FCBetterThanWolves.fcItemMushroomBrown.itemID ||
    			heldID == FCBetterThanWolves.fcItemMushroomRed.itemID || 		
    			heldID == SCDefs.bambooItem.itemID)
    	{
    ...
    public boolean RenderBlock(RenderBlocks render, int x, int y, int z) {
		...
		SCTileEntityFlowerPot potTile = (SCTileEntityFlowerPot) render.blockAccess.getBlockTileEntity(x, y, z);
		...
		//Bamboo renders differently than everything else
		else if (storedBlockID == SCDefs.bambooRoot.blockID) {
				render.SetRenderAllFaces(true);
				
				render.setRenderBounds(6/16F, 6/16F, 6/16F, 10/16F, 1F, 10/16F);
				render.renderStandardBlock(SCDefs.bambooRoot, x, y, z);
				
				Icon icon = Block.blocksList[SCDefs.bambooRoot.blockID].getBlockTextureFromSide(7);
				SCUtilsRender.RenderCrossedSquaresWithTexture(render, SCDefs.bambooRoot, x, y, z, icon, false);
