...
public class FCUtilsInventory
{
		...
		int iMinSlotIndex, int iMaxSlotIndex )
	{
	    if ( !itemstack.isItemDamaged() )
	    {
	        ...
	        {
	        	//System.out.println("attempting to merge: true");
	            return true;
	        }
	    }
    ...
    static public boolean AddItemStackToInventoryInSlotRange2( IInventory inventory, ItemStack itemstack,
		int iMinSlotIndex, int iMaxSlotIndex )
	{
	    if ( !itemstack.isItemDamaged() )
	    {
	        if ( AttemptToMergeWithExistingStacksInSlotRange2( inventory, itemstack, iMinSlotIndex, iMaxSlotIndex ) )	        
	        {
	            return true;
	        }
	    }
    ...
    static private boolean CanStacksMerge2( ItemStack sourceStack, ItemStack destStack, int iInventoryStackSizeLimit )
    {
        return ( destStack != null && 
        	destStack.itemID == sourceStack.itemID && 
        	destStack.isStackable() && 
        	destStack.stackSize < 64 && //destStack.stackSize < destStack.getMaxStackSize() && 
        	destStack.stackSize < iInventoryStackSizeLimit && 
    		( !destStack.getHasSubtypes() || destStack.getItemDamage() == sourceStack.getItemDamage() ) &&
			ItemStack.areItemStackTagsEqual( destStack, sourceStack ) );
    }
    ...
    static private int FindSlotToMergeItemStackInSlotRange2( IInventory inventory, ItemStack itemStack, int iMinSlotIndex, int iMaxSlotIndex )
    {
    	int iInventoryStackLimit = inventory.getInventoryStackLimit();
        ...
        for ( int i = iMinSlotIndex; i <= iMaxSlotIndex; i++ )
        {
            ...
            if ( CanStacksMerge2( itemStack, tempStack, iInventoryStackLimit ) )
            {
                return i;
            }
        }
    ...
    static private boolean AttemptToMergeWithExistingStacksInSlotRange2( IInventory inventory, ItemStack stack, int iMinSlotIndex, int iMaxSlotIndex )
    {
        int iMergeSlot = FindSlotToMergeItemStackInSlotRange2( inventory, stack, iMinSlotIndex, iMaxSlotIndex );
        
        //System.out.println("slot: " + iMergeSlot);
...
//	        }
	        ...
	        iMergeSlot = FindSlotToMergeItemStackInSlotRange2( inventory, stack, iMinSlotIndex, iMaxSlotIndex );	        
        }
