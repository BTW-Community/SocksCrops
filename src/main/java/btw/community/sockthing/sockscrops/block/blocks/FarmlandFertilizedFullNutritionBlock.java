...
public class FarmlandFertilizedFullNutritionBlock extends FarmlandFullNutritionBlock {
    public FarmlandFertilizedFullNutritionBlock(int blockID, String name) {
        super(blockID, name);
    }
    ...
    public float getNutritionMultiplier() {
        return 2F * NutritionUtils.MULTIPLIER_FULL_NUTRITION;
    }
    ...
    @Override
    @Environment(EnvType.CLIENT)
    public Icon getBlockTexture(IBlockAccess blockAccess, int x, int z, int y, int side) {
        if (secondPass) {
            return getBlockTextureSecondPass(blockAccess, x, z, y, side);
        } else return super.getBlockTexture(blockAccess, x, z, y, side);
    }
    ...
    public void renderBlockAsItem(RenderBlocks renderer, int itemdamage, float brightness) {
        super.renderBlockAsItem(renderer, itemdamage, brightness);
